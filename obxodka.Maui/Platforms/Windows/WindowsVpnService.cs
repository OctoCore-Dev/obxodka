...
internal sealed partial class WindowsVpnService : IVpnService, IDisposable
{
    ...
    {
        var psDel = "Get-NetAdapter -ErrorAction SilentlyContinue | Where-Object Name -like 'Obx*' | Remove-NetAdapter -Confirm:$false -ErrorAction SilentlyContinue; " +
                    "Get-DnsClientNrptRule -ErrorAction SilentlyContinue | Where-Object Comment -eq 'ObxodkaVPN' | Remove-DnsClientNrptRule -Force -ErrorAction SilentlyContinue";
    ...
    public async Task StartVpnAsync(string serverIp, int serverPort)
    {
        ...
        try
        {
            ...
            await SetAdapterConfigAsync(adapter.Name, ip, "255.192.0.0");
