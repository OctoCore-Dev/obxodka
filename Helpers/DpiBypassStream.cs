...
public partial class DpiBypassStream(Stream innerStream) : Stream
{
    ...
    public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        ...
        {
            _firstWrite = false;
            await _innerStream.WriteAsync(buffer.AsMemory(offset, 5), cancellationToken).ConfigureAwait(false);
            await Task.Delay(10, cancellationToken).ConfigureAwait(false);
            await _innerStream.WriteAsync(buffer.AsMemory(offset + 5, count - 5), cancellationToken).ConfigureAwait(false);
        }
        else
        {
