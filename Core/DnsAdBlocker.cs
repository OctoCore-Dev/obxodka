...
public static class DnsAdBlocker
{
    private static readonly FrozenSet<string> t_blockedDomains = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "begun.ru",
        "relap.io",
        "analytics.sberbank.ru",
        "stat.sberbank.ru",
        "telemetry.sberbank.ru",
        "analytics.gosuslugi.ru",
        "stats.tinkoff.ru",
        "telemetry.tbank.ru",
        "stat.vtb.ru",
    ...
    private static byte[] WrapUpstreamDnsPayload(byte[] req, int ipHdrLen, int udpOffset, byte[] upstreamPayload, bool isIpv4)
    {
        ...
        ComputeChecksums(resp, ipHdrLen, udpOffset, totalLen, isIpv4);
        return resp;
    }
    ...
    private static bool IsBlocked(string domain)
    {
        ...
        foreach (var blocked in t_blockedDomains)
        {
            ...
            {
                var prefixLen = domain.Length - blocked.Length;
                if (prefixLen == 0 || domain[prefixLen - 1] == '.')
                {
                    return true;
                }
            }
    ...
    private static void ComputeChecksums(byte[] resp, int ipHdrLen, int udpOffset, int totalLen, bool isIpv4)
    {
        if (isIpv4)
        {
            uint sum = 0;
